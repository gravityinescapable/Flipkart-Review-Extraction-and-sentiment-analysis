...
app=Flask(__name__)
app.config['SEND_FILE_MAX_AGE_FEFAULT']=0
...
def extract_all_reviews(url,clean_reviews,org_reviews,customernames,commentheads,ratings):
    ...
    customernames_=page_html.find_all('p',{'class':'_2sc7ZR_2V5EHH'})
...
def result():
    ...
    proname= page_html.find_all('span',{'class':'B-NuCI'})[0].get_text()
    price = page_html.find_all('div',{'class':'_30jeq3 _16Jk6d'})[0].get_text()
    
    # getting the link of see all reviews button
    all_reviews_url= page.html.find_all('div',{'class':'col JOpGWq'})[0]
    ...
    #start reading reviews and go to the next page once all reviews are read
    while True:
        x= len(clean_reviews)
    ...
    #making a directory of product attributes and saving all the products in it
    d= []
